...
export default function DraftHelper(){
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-xs text-gray-600">Compute updates the lists below for the selected round and pick; it does not auto‑draft or change your roster.</div>
          ...
          return (
            <div key={pos} className="card p-3">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-gray-500"><th>Player</th><th className="text-right">Score</th><th className="text-right">VORP</th><th className="text-right">ADP (FP)</th><th className="text-right">ADP (ESPN)</th><th className="text-right">Reach</th></tr>
                </thead>
                <tbody>
                  {list.map((r:any)=> {
                    ...
                    const scoreBadgeCls = s>=p90 ? 'bg-green-600 text-white' : s>=p75 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'
                    const reachBadgeCls = reach<=0 ? 'bg-green-100 text-green-800' : reach<=1 ? 'bg-yellow-100 text-yellow-800' : reach<=2 ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800'
                    const rowHl = s>=p90 ? 'bg-green-50' : reach>2 ? 'bg-orange-50' : ''
                    return (
                      <tr key={r.player_id} className={`border-t ${rowHl}`}>
                        <td className="py-1">
                          <div className="flex items-center gap-2">
                            <span className="font-medium">{r.name}</span>
                            {r.team && <span className="text-xs bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded">{r.team}</span>}
                            {r.rationale && <span title={r.rationale} className="text-xs text-gray-500">ⓘ</span>}
                          </div>
                        </td>
                        <td className="text-right"><span className={`inline-block px-2 py-0.5 rounded-full ${scoreBadgeCls}`}>{s}</span></td>
                        <td className="text-right"><span className="inline-block px-2 py-0.5 rounded-full bg-blue-100 text-blue-800" title="Value Over Replacement">{r.vorp}</span></td>
                        <td className="text-right text-gray-700">{adpFp}</td>
                        <td className="text-right text-gray-700">{adpEspn}</td>
                        <td className="text-right"><span className={`inline-block px-2 py-0.5 rounded-full ${reachBadgeCls}`}>{reach}</span></td>
                      </tr>
                    )
                  })}
                </tbody>
